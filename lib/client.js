...
var SparqlClient = module.exports = function (endpoint, options) {
  ...
  var responseHandler = function responseHandler(error, response, responseBody, callback) {
    ...
    if (error || response.statusCode >= 300) {
      if(arguments.length === 2 && !callback && typeof(response)==='function' && typeof(callback)==='undefined'){
        callback = response;
      }
      if(error && error.message){
        continuation = nextTick(callback, [new Error("SparQL query failed. " + error.message), null], that);
      } else {
        if(responseBody.indexOf("Virtuoso") !== -1 && responseBody.indexOf("Error") !== -1){
          responseBody = responseBody.split('\n')[0]
        }
        continuation = nextTick(callback, [new Error("SparQL query failed. " + responseBody ), null], that);
      }
